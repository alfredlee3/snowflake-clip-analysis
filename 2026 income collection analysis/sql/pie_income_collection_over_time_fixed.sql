-- ============================================================================
-- PIE Income Collection Over Time - Monthly Progression Analysis (FIXED)
-- ============================================================================
-- PURPOSE: Track how success rates evolve month-by-month over the 8-month
--          window after PIE evaluation
--
-- FIX: Corrected "approved outright" logic to ensure accounts are NOT
--      counted as "approved outright" if they ever had PIE at any statement
--
...
--   - ... up to Month 7: 211-240 days after PIE
-- ============================================================================
...
base_population_overall as (
    ...
    -- FIXED: Approved outright now correctly excludes accounts with PIE
    -- ========================================================================
    select
        clip.account_id,
...
cumulative_success as (
    ...
    select
        ...
        -- FIXED: Accounts that were approved outright (no PIE barrier)
        ...
        count(distinct case
            when base.had_pie_in_month = 0  -- Approved outright (never had PIE)
...
cumulative_success_overall as (
    ...
    -- FIXED: Approved outright = accounts that NEVER had PIE
    -- ========================================================================
    select
        ...
        -- FIXED: Approved outright = accounts that NEVER had PIE at any Stmt 18+
        count(distinct case when base.had_pie = 0 then base.account_id end) as approved_outright_count,
        ...
        -- SUCCESS COUNT: Approved Outright (never PIE) OR PIE with Income
        count(distinct case
            when base.had_pie = 0  -- Never had PIE at any statement
...
cumulative_success_42plus as (
    ...
    select
        ...
        -- FIXED: Approved outright = accounts that NEVER had PIE at any Stmt 42+
        count(distinct case when base.had_pie = 0 then base.account_id end) as approved_outright_count,
...
--
-- KEY FIX:
--   "Approved Outright" now correctly means: accounts that were NEVER PIE
--   at any statement in the cohort (not "ever approved at any statement")
--
-- INTERPRETATION:
...
--
-- VERIFICATION:
--   approved_outright_count + pie_total_count should equal total_population
--   (every account is either "never PIE" or "had PIE", mutually exclusive)
-- ============================================================================
